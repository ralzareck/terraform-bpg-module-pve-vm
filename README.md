...
| <a name="input_vm_type"></a> [vm\_type](#input\_vm\_type) | The creation type of the VM. Can be 'clone' or 'img' | `string` | `"clone"` | $\color{red}{no}$ |
...
| <a name="input_vm_start"></a> [vm\_start](#input\_vm\_start) | The start settings for the VM. | <pre>object({<br/>    on\_deploy  = bool<br/>    on\_boot    = bool<br/>    order      = optional(number, 0)<br/>    up\_delay   = optional(number, 0)<br/>    down\_delay = optional(number, 0)<br/>  })</pre> | <pre>{<br/>  "on\_boot": false,<br/>  "on\_deploy": true<br/>}</pre> | $\color{red}{no}$ |
| <a name="input_vm_bios"></a> [vm\_bios](#input\_vm\_bios) | The BIOS Implementation of the VM. | `string` | `"ovmf"` | $\color{red}{no}$ |
...
| <a name="input_vm_os"></a> [vm\_os](#input\_vm\_os) | The Operating System configuration. | `string` | `"l26"` | $\color{red}{no}$ |
| <a name="input_vm_cpu"></a> [vm\_cpu](#input\_vm\_cpu) | VM CPU Configuration. | <pre>object({<br/>    type      = optional(string, "host")<br/>    cores     = optional(number, 2)<br/>    units     = optional(number)<br/>  })</pre> | `{}` | $\color{red}{no}$ |
| <a name="input_vm_mem"></a> [vm\_mem](#input\_vm\_mem) | VM Memory Configuration. | <pre>object({<br/>    dedicated = optional(number, 2048)<br/>    floating  = optional(number)<br/>    shared    = optional(number)<br/>  })</pre> | `{}` | $\color{red}{no}$ |
| <a name="input_vm_display"></a> [vm\_display](#input\_vm\_display) | VM Display Configuration. | <pre>object({<br/>    type      = optional(string, "std")<br/>    memory    = optional(number, 16)<br/>  })</pre> | `{}` | $\color{red}{no}$ |
...
| <a name="input_vm_efi_disk"></a> [vm\_efi\_disk](#input\_vm\_efi\_disk) | The UEFI disk device. | <pre>object({<br/>    datastore\_id      = string<br/>    file\_format       = optional(string, "raw")<br/>    type              = optional(string, "4m")<br/>    pre\_enrolled\_keys = optional(bool, false)<br/>  })</pre> | `n/a` | $\color{green}{yes}$ |
| <a name="input_vm_img_disk"></a> [vm\_img\_disk](#input\_vm\_img\_disk) | The CDROM interface for the VM. | <pre>object({<br/>    interface    = string<br/>    datastore\_id = string<br/>    size         = number<br/>    file\_format  = optional(string, "qcow2")<br/>    iothread     = optional(bool, true)<br/>  })</pre> | `null` | $\color{red}{no}$ |
| <a name="input_vm_disk"></a> [vm\_disk](#input\_vm\_disk) | VM Disks configuration. | <pre>map(object({<br/>    datastore\_id = string<br/>    size         = number<br/>    file\_format  = optional(string, "raw")<br/>    iothread     = optional(bool, true)<br/>  }))</pre> | `{}` | $\color{red}{no}$ |
| <a name="input_vm_net_ifaces"></a> [vm\_net\_ifaces](#input\_vm\_net\_ifaces) | VM network interfaces configuration. | <pre>map(object({<br/>    bridge     = string<br/>    enabled    = optional(bool, true)<br/>    firewall   = optional(bool, true)<br/>    mac\_addr   = optional(string)<br/>    model      = optional(string, "virtio")<br/>    mtu        = optional(number, 1500)<br/>    rate\_limit = optional(string)<br/>    vlan\_id    = optional(number)<br/>    ipv4\_addr  = string<br/>    ipv4\_gw    = string<br/>  }))</pre> | `{}` | $\color{red}{no}$ |
| <a name="input_vm_init"></a> [vm\_init](#input\_vm\_init) | Initial configuration for the VM | <pre>object({<br/>    datastore\_id = optional(string)<br/>    interface    = optional(string, "ide0")<br/>    user         = optional(object({<br/>      name         = optional(string)<br/>      password     = optional(string)<br/>      keys         = optional(list(string))<br/>    }))<br/>    dns          = optional(object({<br/>      domain       = optional(string)<br/>      servers      = optional(list(string))<br/>    }))<br/>  })</pre> | `{}` | $\color{red}{no}$ |
